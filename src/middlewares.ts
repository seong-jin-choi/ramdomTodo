...
export const localsMiddleware = (req: Request, res: Response, next: NextFunction) => {
  ...
  res.locals = {
    ...
    // 날짜 형식 변환
    dateFormatYMD: (date: any) => moment(date).tz("Asia/Seoul").format("YYYY-MM-DD"),
    dateFormatYMDHm: (date: any) => moment(date).tz("Asia/Seoul").format("YYYY-MM-DD HH:mm"),
    dateFormatYMDHms: (date: any) => moment(date).tz("Asia/Seoul").format("YYYY-MM-DD HH:mm:ss"),
    // 해당하는 문자열 모두 치환
    replaceAll: (str: string, searchStr: string, replaceStr: string) => str.split(searchStr).join(replaceStr),
    // 배열 Random 섞기
    ...
    arrIncludesStr: (arr: any[], str: string) => {
